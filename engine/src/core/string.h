/*
    @file string.h
    @brief Интерфейс для работы со строками.
    ...
    @note Предоставляет:
            - Базовые операции со строками (длина, копирование, сравнение)
            - Безопасное форматирование строк с поддержкой va_list
            - Управление памятью для строковых операций
            - Кроссплатформенные обертки над нативными строковыми функциями
