/*
    ...
    @note Для использования системы менеджмента памяти необходимо предварительно инициализировать
          базовую подсистему памяти платформы вызовами:
            - platform_memory_initialize() и platform_memory_shutdown()
*/
...
typedef struct memory_format {
    // @brief Название единицы измерения (например, "B", "KiB", "MiB", "GiB").
    const char* name;
