...
typedef struct application_context {
    ...
    f64 sleep_error_time;   // Ошибка сна (недоспал / много спал).
...
bool application_initialize(const application_config* config)
{
    ...
    context->window_time = 0.0;
    context->render_timr = 0.0;
    context->frame_time = 0.0;
    context->sleep_time = 0.0;
    context->sleep_actual_time = 0.0;
    context->sleep_error_time = 0.0;
...
void application_run()
{
    ...
    while(context->is_running)
    {
        ...
        context->window_time = timer_elapsed(&frame_timer);
        ...
        if(!context->is_suspended)
        {
            ...
            // NOTE: Обновляется раз в секунду.
            if(frame_time_accumulator >= 1.0)
            {
                context->current_fps = frame_count;
                ...
                LOG_DEBUG(
                    ...
                    sleep_ft.unit, sleep_actual_ft.amount, sleep_actual_ft.unit, sleep_error_ft.amount, sleep_error_ft.unit, context->current_fps
                );
            }

            // TODO: Временно, т.к. нет просчета физики и др.
            context->render_timr = timer_elapsed(&frame_timer) - context->window_time;
        }

        context->frame_time = timer_elapsed(&frame_timer);
        context->sleep_time = context->target_frame_time - context->frame_time - context->sleep_error_time;
        context->sleep_error_time = 0.0;

        if(context->sleep_time >= 0.001)
        {
            platform_thread_sleep((u32)(context->sleep_time * 1000.0));

            // Вычисление ошибки и ограничение в разумных пределах.
            context->sleep_actual_time = timer_elapsed(&frame_timer) - context->frame_time;
            context->sleep_error_time = context->sleep_actual_time - context->sleep_time;
            context->sleep_error_time = CLAMP(
                context->sleep_error_time, -context->target_frame_time * 0.5, context->target_frame_time * 0.5
            );
        }

        // TODO: Для корректного расчета статистики. Временно.
        timer_get_format(context->window_time, &window_ft);
        timer_get_format(context->render_timr, &render_ft);
        timer_get_format(context->frame_time, &frame_ft);
        timer_get_format(context->sleep_time, &sleep_ft);
        timer_get_format(context->sleep_actual_time, &sleep_actual_ft);
        timer_get_format(context->sleep_error_time, &sleep_error_ft);
    }
}
